...
impl JsState {
    pub fn new() -> Self {
        ...
        block_on(|_reactor| async {
            ...
            let resolver = BuiltinResolver::default().with_module("bundle/script_module").with_module("sample").with_module("bundle/user_script");
            let resolver = crate::modules::add_native_module_resolvers(resolver);
            let resolver = crate::builtin::add_module_resolvers(resolver);

            let loader = (
                BuiltinLoader::default().with_module("bundle/script_module", JS_MODULE).with_module("sample", JS_MODULE).with_module("bundle/user_script", USER_MODULE),
           ...
           Module::import(&ctx, "bundle/user_script")
                ...
                // let result: rquickjs::Result<()> = ctx.eval(MODULE);
                // match result {
                //     Ok(_) => (),
                //     Err(e) => {
                //         panic!("Failed to evaluate module: {e:?}");
                //     }
                // }
            })
            .await;
